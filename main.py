...
import subprocess
import logging
import shutil # Temizlik için gerekli
from telebot import TeleBot, types

# --- AYARLAR ---
BOT_TOKEN = "8810318834:AAGO6mwVERb8P82yOYyi_hkUPKkn_HKsUBU"
ADMIN_ID = 8629520501
...
def admin_actions(call):
    ...
    if action == "app":
        os.rename(os.path.join(TEMP_DIR, f"{uid}_{name}"), os.path.join(BASE_DIR, f"{uid}_{name}"))
        bot.send_message(int(uid), f"🎉 `{name}` dosyanız onaylandı!", parse_mode="Markdown")
